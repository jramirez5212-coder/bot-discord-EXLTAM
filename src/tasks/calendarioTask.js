...
const EVENTOS = [
  { hora: "15:00", nombre: "Torneo 1v1",                     tipo: "torneo",      puntos: "x1 pts", rank: "F1", jugadores: 2  },
  { hora: "15:30", nombre: "Torneo Bandas 2v2",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 4  },
  { hora: "16:00", nombre: "Torneo Bandas 2v2",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 4  },
  { hora: "16:30", nombre: "Torneo Bandas 2v2",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 4  },
  { hora: "17:30", nombre: "Torneo Bandas 3v3",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 6  },
  { hora: "18:30", nombre: "Torneo 1v1",                     tipo: "torneo",      puntos: "x1 pts", rank: "F1", jugadores: 2  },
  { hora: "19:00", nombre: "Tanda de Tormentas (8 tormentas)",tipo: "tormenta",    puntos: null,     rank: "F7", jugadores: null },
  { hora: "20:00", nombre: "Torneo Bandas 4v4",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 8  },
  ...
  { hora: "23:00", nombre: "Torneo Bandas 5v5",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 10 },
  { hora: "23:30", nombre: "Torneo Bandas 4v4",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 8  },
  { hora: "00:45", nombre: "Torneo Bandas 4v4",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 8  },
  { hora: "01:30", nombre: "Torneo 1v1",                     tipo: "torneo",      puntos: "x1 pts", rank: "F1", jugadores: 2  },
  { hora: "02:00", nombre: "Torneo Bandas 3v3",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 6  },
  { hora: "02:45", nombre: "Torneo Bandas 3v3",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 6  },
  { hora: "03:30", nombre: "Torneo Bandas 2v2",               tipo: "torneo",      puntos: "x1 pts", rank: "F4", jugadores: 4  },
];
...
// ── TORNEOS NORMALES: embed con inscripciones ─────────────────────────────────
async function lanzarTorneoNormal(evento, client) {
  const canal = await client.channels.fetch(CANAL_CMD_TORNEO).catch(() => null);
  if (!canal) return;

  const key = `${evento.hora}-${evento.nombre}`;
  inscripcionesActivas.set(key, { inscritos: new Set(), maxJugadores: evento.jugadores });

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`inscribir_torneo:${key}`).setLabel("✅ Inscribirme").setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(`salir_torneo:${key}`).setLabel("❌ Salir").setStyle(ButtonStyle.Danger)
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `👥 **Jugadores:** 0 / ${evento.jugadores ?? "∞"}\n\n` +
      `Presiona **✅ Inscribirme** para unirte.`
    )
    .setTimestamp();

  await canal.send({ content: `<@&${ACTIVITY_ROLE_ID}> ¡Torneo disponible!`, embeds: [embed], components: [row] });

  // Cerrar inscripciones después de 10 minutos
  setTimeout(async () => {
    const data = inscripcionesActivas.get(key);
    if (!data) return;
    const listaInscritos = data.inscritos.size > 0
      ? [...data.inscritos].map(id => `<@${id}>`).join(", ")
      : "Nadie se inscribió.";
    const embedCierre = new EmbedBuilder()
      .setColor(0xe74c3c)
      .setTitle(`🔒 Inscripciones cerradas — ${evento.nombre}`)
      .setDescription(`**Inscritos (${data.inscritos.size}):** ${listaInscritos}`)
      .setTimestamp();
