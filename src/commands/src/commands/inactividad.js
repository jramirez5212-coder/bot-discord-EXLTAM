...
async function handleInactividad(message) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`btn_inactivo:${message.author.id}`).setLabel("Llenar formulario").setStyle(ButtonStyle.Primary).setEmoji("📋")
  );
...
async function handleInactividadButton(interaction) {
  ...
  const modal = new ModalBuilder().setCustomId("modal_inactividad").setTitle("📋 Justificación de Inactividad");
  modal.addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("razon").setLabel("¿Cuál es tu razón de inactividad?").setStyle(TextInputStyle.Paragraph).setPlaceholder("Ej: Viaje, trabajo, salud...").setRequired(true)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("desde").setLabel("¿Desde qué fecha? (YYYY-MM-DD)").setStyle(TextInputStyle.Short).setPlaceholder("Ej: 2026-05-15").setRequired(true)),
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("hasta").setLabel("¿Hasta qué fecha? (YYYY-MM-DD)").setStyle(TextInputStyle.Short).setPlaceholder("Ej: 2026-05-20").setRequired(true))
  );
  ...
  try {
    ...
    if (canal) {
      const embed = new EmbedBuilder()
        ...
        .setTimestamp().setFooter({ text: `ID: ${interaction.user.id}` });
      await canal.send({ embeds: [embed] });
    }
  } catch (err) { console.error("[INACTIVIDAD] Error:", err); }

  await interaction.reply({ content: `✅ Justificación enviada. No recibirás advertencias hasta el **${hasta}**.`, ephemeral: true });
}
