...
const { ACTIVITY_ROLE_ID, LOGO_URL,
        CANAL_CMD_TORNEO }            = require("../config");
...
async function handleTorneoInteraction(interaction, client) {
    ...
    modal.addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId("nombre").setLabel("Nombre del torneo")
          .setStyle(TextInputStyle.Short).setPlaceholder("Ej: Torneo de Drift").setRequired(true)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId("cupo").setLabel("¿Cuántos jugadores? (ej: 2, 3, 4, 6...)")
          .setStyle(TextInputStyle.Short).setPlaceholder("Ej: 4").setRequired(true)
      )
    );
  ...
  if (interaction.isModalSubmit() && interaction.customId === "modal_torneo") {
    ...
    // Mencionar el rol FUERA del embed para que llegue la notificación
    await interaction.reply({
      content: `<@&${ACTIVITY_ROLE_ID}> 🏆 **¡Nuevo torneo abierto: ${nombre}!**`,
      embeds:  [embed],
      components: [row]
    });
  ...
  if (interaction.isButton() && interaction.customId === "unirse_torneo") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(lleno ? 0xe74c3c : 0x39FF14).setThumbnail(LOGO_URL)
      .setDescription(
        lleno
    ...
    if (lleno) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          for (const uid of torneo.jugadores) {
            try { const m = await guild.members.fetch(uid); await m.roles.remove(ROL_TORNEO_ID); } catch {}
          }
          const canal = await client.channels.fetch(torneo.channelId);
